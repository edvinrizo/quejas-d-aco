...
class TipoComercio (models.Model):
    id_tipo_comercio = models.IntegerField('ID Tipo Comercio', db_column='id_tipo_comercio_PK', primary_key=True, editable=False)
    tipo_comercio = models.CharField('Tipo Comercio', db_column='tipo_comercio', max_length=50, unique=True)

    def __str__(self):
        return '%s' % (self.tipo_comercio)
...
class Comercio (models.Model):
    id_comercio = models.IntegerField('ID Comercio', primary_key=True, editable=False, db_column='id_comercio_PK')
    ...
    def __str__(self):
        return '%s %s' % (self.razon_comercial, self.nit)
...
class Sucursal (models.Model):
    ...
    id_comercio = models.ForeignKey(Comercio, verbose_name='Comercio', db_column='id_comercio_FK', on_delete='CASCADE')
    ...
    #                             db_column='id_depto_FK', on_delete='CASCADE')
    id_muni = models.ForeignKey(Municipio, help_text='Municipio donde se ubica el comercio', verbose_name='Municipio',
                                db_column='id_muni_FK', on_delete='CASCADE')
    zona = models.IntegerField('Zona', db_column='zona', unique=True)
    ...
    def __str__(self):
        return '%s %s' % (self.id_comercio, self.nombre_sucursal)
